...
VDIR       := $(OUT_DIR)/vlog
BSC_VFLAGS := +RTS -K512M -RTS -verilog -p $(BSC_PATH) -bdir $(VDIR) -vdir $(VDIR) -info-dir $(VDIR)

YOSYS       ?= yosys
BLUESPECDIR ?= $(abspath $(dir $(shell command -v $(BSC)))/..)
BSV_VLIB    := $(BLUESPECDIR)/lib/Verilog

VIVADO ?= vivado
XPART  ?= xc7a35tcpg236-1
SYNDIR := syn
VIVDIR := $(OUT_DIR)/vivado
SYN_V  := $(VDIR)/mkSpeculaCore.syn.v
...
.PHONY: all run image disasm clean rvctest csrtest uarttest mmutest m18test virtiotest interactive verilog verilog-check vivado-syn
...
$(VDIR)/mkSpeculaCore.v: $(SRC_DIR)/SpeculaCore.bsv $(IMG_BSV) | $(OUT_DIR)
	@mkdir -p $(VDIR)
	@echo "[verilog] Generating $(TOP) RTL -> $@"
	$(BSC) $(BSC_VFLAGS) -u -g $(TOP) $(SRC_DIR)/SpeculaCore.bsv

verilog: $(VDIR)/mkSpeculaCore.v

verilog-check: $(VDIR)/mkSpeculaCore.v
	@mkdir -p $(VDIR)
	@printf '%s\n' \
	  '// GENERATED by "make verilog-check" - not tracked.' \
	  '(* blackbox *)' \
	  'module RegFileLoad #(parameter file="", parameter addr_width=1, parameter data_width=1,' \
	  '                     parameter lo=0, parameter hi=1, parameter binary=0)' \
	  '  (input CLK,' \
	  '   input  [addr_width-1:0] ADDR_IN, ADDR_1, ADDR_2, ADDR_3, ADDR_4, ADDR_5,' \
	  '   input  [data_width-1:0] D_IN, input WE,' \
	  '   output [data_width-1:0] D_OUT_1, D_OUT_2, D_OUT_3, D_OUT_4, D_OUT_5);' \
	  'endmodule' > $(VDIR)/bsv_blackbox.v
	$(YOSYS) -l $(VDIR)/yosys-check.log -p "read_verilog -sv $(VDIR)/bsv_blackbox.v $(BSV_VLIB)/SizedFIFO.v $(BSV_VLIB)/FIFO2.v $(VDIR)/mkSpeculaCore.v; hierarchy -top $(TOP); setattr -set keep 1 $(TOP)/w:*; proc; opt; check; stat"


$(SYN_V): $(VDIR)/mkSpeculaCore.v
	@awk '1; /^  input  RST_N;$$/ { \
	  print "  wire [31:0] TASK_fopen___d3 = 0;   // synth shim (not architecture)"; \
	  print "  wire [31:0] x__h8951 = 0;          // synth shim (not architecture)" }' \
	  $< > $@
	@echo "[vivado] synthesis-adjusted RTL -> $@"

vivado-syn: $(SYN_V) $(SYNDIR)/specula_fpga_top.v $(SYNDIR)/bsv_blackbox.v $(SYNDIR)/specula_syn.xdc $(SYNDIR)/specula_synth.tcl | $(OUT_DIR)
	@mkdir -p $(VIVDIR)
	REPO=$(CURDIR) SYN_V=$(SYN_V) BSV_VLIB=$(BSV_VLIB) XPART=$(XPART) SYNDIR=$(SYNDIR) VIVDIR=$(VIVDIR) \
	  $(VIVADO) -mode batch -nojournal -log $(VIVDIR)/vivado.log -source $(SYNDIR)/specula_synth.tcl
	@rm -rf .Xil
	@echo "[vivado] reports in $(VIVDIR)/"
